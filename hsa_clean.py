...
def hsa(parm_file,traj_file,lig_file,num_frames:str,workDir,water_model,start_frame,cluster_file=None,dist=10):
    # Check for clashing file names in cwd
    if os.path.isdir(workDir): 
        print(f'Error, {workDir} already exists. Please change output prefix')
        return
    else:
        os.makedirs(workDir)
    ...
    # Run HSA
    if cluster_file:
        result=subprocess.run("run_hsa -i "+parm_file+" -t "+traj_file+" -l "+lig_file+" -f "+num_frames+" -d "+dist+" -c "+cluster_file+" -s "+start_frame+" -o "+"results",shell=True)
    else: 
        result=subprocess.run("run_hsa -i "+parm_file+" -t "+traj_file+" -l "+lig_file+" -f "+num_frames+" -d "+dist+" -s "+start_frame+" -o "+"results",shell=True)
    if result.returncode != 0:
        print("Error running HSA:")
        print(result.stderr)
    else:
        os.rename("SSTMap_HSA", workDir)
        print(f"HSA successful. Raw outputs written to {workDir}")

    # Post HSA analysis
    # Classifying based on entropy/enthalpy
    Eww=watermodels[water_model]
    with open(os.path.join(workDir,'results_hsa_summary.txt'), 'r') as f:
    ...
    with open(os.path.join(script_dir,'displaced_kde.pkl'),'rb') as f: displaced_kde=pl.load(f)
    with open(os.path.join(script_dir,'conserved_kde.pkl'),'rb') as f: conserved_kde=pl.load(f)
    def prob(wat):
        d=36/54
        c=18/54
    ...
    if cluster_file:
        ...
        hsa_results.to_csv(os.path.join(workDir,'results_HSA_results.txt'),index=True, sep=' ',quoting=csv.QUOTE_NONE)
    else: 
...
def main():
    ...
    required.add_argument('-o','--workDir',help='workDir',required=True)
